因为 Redis 7.2 docker 镜像里面没有配置文件，所以需要去 redis 官网下载一个复制里面的 redis.conf

```yaml
version: '3'
services:
  redis:
    image: registry.cn-qingdao.aliyuncs.com/xuxiaoweicomcn/redis:7.2.4
...
需要修改 `requirepass xxxxxx`，`xxxxxx`为自定义密码

注释掉 `# bind 127.0.0.1 -::1`，否则外部无法连接

`appendonly yes` 持久化机制
